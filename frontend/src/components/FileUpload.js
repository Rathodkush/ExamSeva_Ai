...
export default function FileUpload({ setResults }) {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const resp = await axios.get('http://localhost:4000/api/auth/me', { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleFiles = async (e) => {
    ...
    if (selected.length > 0) {
      try {
        ...
        const resp = await axios.post('http://localhost:4000/api/upload/preview', form, { headers, timeout: 30000 });
  ...
  const upload = async () => {
    ...
    try {
      ...
      if (err.code === 'ECONNABORTED' || err.response?.status === 504) {
        ...
        errorMessage = "Cannot connect to server. Please make sure the backend server is running on http://localhost:4000";
          ...
          <button
            onClick={async () => {
              ...
              try {
                ...
                const headers = {};
                if (token) headers['Authorization'] = `Bearer ${token}`;
                const resp = await axios.post('http://localhost:4000/api/upload/enhance', form, { headers, timeout: 120000 });
                if (resp.data) {
